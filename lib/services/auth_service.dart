...
class AuthSession {
  ...
  final int since;
  AuthSession({required this.userId, required this.username, required this.since});

  Map<String, dynamic> toJson() => {'userId': userId, 'username': username, 'since': since};
  factory AuthSession.fromJson(Map<String, dynamic> j) => AuthSession(
        userId: j['userId'] as String,
        username: j['username'] as String,
        since: j['since'] as int? ?? 0,
      );
}
...
class AuthService {
  ...
  Future<AuthSession?> restore() async {
    ...
    try {
      ...
      _session = AuthSession.fromJson(j);
      _api.setSession(userId: _session!.userId, username: _session!.username);
      return _session;
    } catch (_) {
      return null;
    }
  ...
  Future<AuthSession> login(String username, String password) async {
    ...
    if (userId.isEmpty) throw ApiException(-1, '登录返回数据异常');
    final s = AuthSession(userId: userId, username: uname, since: DateTime.now().millisecondsSinceEpoch);
    await _persist(s);
    return s;
  }
  ...
  Future<AuthSession> register(String username, String password) async {
    ...
    final userId = (data['userId'] ?? '').toString();
    final uname = (data['username'] ?? username).toString();
    if (userId.isEmpty) throw ApiException(-1, '注册返回数据异常');
    final s = AuthSession(userId: userId, username: uname, since: DateTime.now().millisecondsSinceEpoch);
    await _persist(s);
    return s;
  }
  ...
  Future<void> _persist(AuthSession s) async {
    _session = s;
    _api.setSession(userId: s.userId, username: s.username);
    final prefs = await SharedPreferences.getInstance();
