      ...
      body: ListView(
        ...
        children: [
          ...
          _Section(title: '关于', children: [
            ...
            _Item(
              icon: Icons.info_outline, label: '版本 v1.0.3',
              onTap: () => showRecallToast(context, 'v1.0.3 · 个人知识管理平台'),
            ),
          ]),
...
class _KeySetupPageState extends ConsumerState<KeySetupPage> {
  ...
  Future<void> _verify() async {
    ...
    try {
      // 调用 user-api /api-keys 接口：后端会先 pingZhipu 验证 Key 有效性，再加密存储
      ...
      if (mounted) {
        setState(() {
          _status = 'error';
          _error = e.toString().replaceFirst('ApiException', '').replaceAll(RegExp(r'^\(\d+,\s*'), '').replaceAll(RegExp(r'\)$'), '');
        });
      }
    }
  }
