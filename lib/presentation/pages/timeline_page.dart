...
class _TimelinePageState extends ConsumerState<TimelinePage> {
  ...
  void initState() {
    ...
    // 轮询 pending items 状态（每 5 秒检查一次，有 pending 才刷新）
    _pollTimer = Timer.periodic(const Duration(seconds: 5), (_) {
      if (!mounted) return;
      final items = ref.read(itemsControllerProvider).items;
      final hasPending = items.any((i) => i.status == ItemStatus.pending);
      if (hasPending) {
  ...
  Widget build(BuildContext context) {
    ...
    final items = _filterStatus == null ? allItems : allItems.where((i) => i.status == _filterStatus).toList();
      ...
      appBar: AppBar(
        ...
        bottom: PreferredSize(
          ...
          child: Padding(
            ...
            child: Row(children: [
              _statusChip('全部', null),
              _statusChip('已整理', ItemStatus.organized),
              _statusChip('整理中', ItemStatus.pending),
              _statusChip('失败', ItemStatus.failed),
            ]),
          ),
        ),
        actions: [
          PopupMenuButton<String>(
            ...
            itemBuilder: (_) => const [
              ...
              PopupMenuItem(value: 'views_desc', child: Text('回看最多')),
            ],
          ),
          IconButton(
  ...
  Widget _statusChip(String label, ItemStatus? status) {
    final active = _filterStatus == status;
    return Padding(
      padding: const EdgeInsets.only(right: 6),
      child: ActionChip(
        label: Text(label, style: TextStyle(fontSize: AppFonts.xs, fontWeight: active ? FontWeight.w700 : FontWeight.w400)),
        backgroundColor: active ? AppColors.primary500.withOpacity(0.2) : null,
        side: BorderSide(color: active ? AppColors.primary500 : Colors.grey.withOpacity(0.3)),
